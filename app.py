...
from util_helper.preproc import resize_img, preproc_img, filter_smooth
import subprocess
import shutil
...
if image_source and st.button("🔍 Cek Nutrisi"):
    ...
    for result in results:
        if result.boxes is not None and len(result.boxes) > 0:
            ...
            resized_img = resize_img(crop_bgr, target_char_height=30)
            filtered_img = filter_smooth(resized_img,10,150,75)
            # resized_img = preproc_img(crop_bgr)
            temp_path = "processed_tmp.tif"
            Image.fromarray(filtered_img).save(temp_path, dpi=(300, 300))
